...
jQuery(function ($) {
    ...
    async function getData() {
        ...
        let formDiv = await FormGenerator.formGenerator(zipperShadeFormFields);
        ...
        orderItems[0].Mounting_type === 'Ceiling' ? $('#FabricDirection').parent().css('display', "block") : $('#FabricDirection').parent().css('display', "none")
        ...
        zipperShadeFields.forEach(item => {
        ...
        $('#MountType').on('change', async function () {
            $('#MountType').val() === 'Ceiling' ? $('#FabricDirection').parent().css('display', "block") : $('#FabricDirection').parent().css('display', "none")
        })
        ...
        $('#backToForm').on('click', function () {
            $('#zipperShadeReduction').remove()
            $('#zipperShadeMeasureSheet').remove()
            $('#backToForm').hide()
            $('#printBox').hide()
            ...
            if (checkReduceRequiredFields()) {
                ...
                if (isCalculated) {
                    if (isUpdatedAfterCalculated) {
                        ...
                        reductionObj = await zipperShadeRequestToCustomFunction('reduction')
            ...
            if (checkReduceRequiredFields()) {
                ...
                if (isCalculated) {
                    if (isUpdatedAfterCalculated) {
                        ...
                        obj = await zipperShadeRequestToCustomFunction('measure')
        ...
        $('#proceedWithoutCalculation').on('click', async function () {
            if ($('#hiddenCalculationType').text() == 'reduction') {
                ...
                let reductionObj = await zipperShadeRequestToCustomFunction('reduction')
                if (reductionObj) {
                    await setValuesToZoho(reductionObj, 'reduction') && createReductionSheet(reductionObj)
                }
            }
            if ($('#hiddenCalculationType').text() == 'measure') {
                $('#calculationErrorModal').css('display', 'none')
                let obj = await zipperShadeRequestToCustomFunction('measure')
                if (obj) {
                    // await ReductionNumberCreator();
                    obj['Number'] = $('#LocationNumber').val() + ' of ' + reductionNumber.total + ' ' + (obj.Location ? '(' + obj.Location + ')' : '');
                    console.log(obj, "obj");
        ...
        async function createMeasureSheet(measureObject) {
            ...
            let printDiv = $(`
            <div id='zipperShadeMeasureSheet' >
                <div class="mainInfoWrapper zippershademainInfoMargin">
                    <div class="infoCol_1">
                        ${measureCreator.mainMeasureTabel(MeasureTabelInfo, measureObject, MeasureExtraFields)}
                        ${measureCreator.measuresheetNoteSection(measureObject)}
                        ${measureCreator.measuresheetImage(measureObject)}
                    </div>    
                    <div class="infoCol_2">
                    ${measureCreator.measuresheetMountingTabelZipperShade(measureObject)}
                    ${measureCreator.measuresheetOtherInfoTabel(MeasureOtherInfoFields, measureObject)}
                    ${measureCreator.measuresheetCrank(measureObject)}
                    ${measureCreator.measuresheetInstallInfo(order[0])}
                    </div>
                </div>         
                <div class='footerRow footerStyle zippershadeMeasureMargin' >
        ...
        async function createReductionSheet(printObject) {
            ...
            Reductionsheets['Zipper Shade'](printObject, orderItems[0], order[0], installContactInfo[0], contactInfo[0], propertyInfo[0], false)
        ...
        async function calculateFunction() {
            ...
            let res = await zipperShadeRequestToCustomFunction('calculate')
        ...
        async function zipperShadeRequestToCustomFunction(type) {
            let reductionObject = {
                functionType: type,
                Number: "",
            ...
            zipperShadeFields.forEach(item => {
                reductionObject[item.zohoName] = item.type !== 'checkbox' ? $('#' + item.id).val() : $('#' + item.id).is(':checked')
            })
            let func_name = "zippershadereduction1";
            let req_data = {
                "arguments": JSON.stringify({
                    "zipperShadeReductionInput": reductionObject
                })
            };
