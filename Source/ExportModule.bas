...
' ==========================================================
Public Sub ExportRawDoorInputsToCSV()

    Dim db As DAO.Database
    Dim rsDoors As DAO.Recordset
    ...
    ' -------------------------------------------------------
    ' Newest 10000 doors by Issue Date
    ' -------------------------------------------------------
    Set rsDoors = db.OpenRecordset( _
        "SELECT TOP 10000 [Door ID] " & _
        "FROM [Door] " & _
        "WHERE [Issue Date] Is Not Null " & _
        "ORDER BY [Issue Date] DESC", dbOpenSnapshot)

    count = 0

    While Not rsDoors.EOF

        doorID = rsDoors![Door ID]
...
' ==========================================================
Public Sub ExportDoorComponentsToCSV()
    ...
    sqlDoors = "SELECT TOP 10000 [Door ID] " & _
               "FROM [Door] " & _
               "WHERE [Issue Date] Is Not Null " & _
               "ORDER BY [Issue Date] DESC;"

    Set rsDoors = db.OpenRecordset(sqlDoors, dbOpenSnapshot)
